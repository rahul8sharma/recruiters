    ...
    if ($('div.pagination').size() > 0){
      ...
      $('.pagination_links').append(
        "<div class='clearfix live_pagination'><div>" +
          "<a class='more_links'><span class='icon'>"+ settings.linkTitle + "</span></a>" +
        "</div></div>"
      );

      $('.pagination_links a.more_links').first().addClass(settings.extraClass);

//      $.extend($('.pagination_links a.more_links').first(), settings.extraCss);
