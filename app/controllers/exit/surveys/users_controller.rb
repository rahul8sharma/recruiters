class Exit::Surveys::UsersController < ApplicationController
  ...
  layout 'exit'
  def bulk_upload
    s3_key = "exit/survey_users/#{@survey.id}_#{Time.now.strftime("%d_%m_%Y_%H_%M_%S_%P")}"
    if !params[:bulk_upload] || !params[:bulk_upload][:file]
      flash[:error] = "Please select a csv file."
      redirect_to add_users_bulk_company_exit_survey_url(company_id: @company.id,id: @survey.id,candidate_stage: params[:candidate_stage]) and return
    end
  ...
  def add_users
    ...
    if request.put?
      ...
      if params[:users].empty?
        flash[:error] = "Please add at least 1 User to send the survey. You may also select 'Add Users Later' to save the survey and return to the Exit Listings."
  ...
  def send_reminder
    if request.get?
      ...
      @user_survey = Vger::Resources::Exit::UserSurvey.send_reminder(params.merge(:survey_id => params[:id], :id => params[:user_survey_id]))
      flash[:notice] = "Reminder was sent successfully!"
      redirect_to users_company_exit_survey_path(:company_id => params[:company_id], :id => params[:id])
    end
  end

  def bulk_send_survey_to_users
    Vger::Resources::Exit::UserSurvey\
    ...
    if request.put?
      ...
      params[:selected_users].each do |user_id,on|
        ...
        unless user_survey
          ...
          user_survey = Vger::Resources::Exit::UserSurvey.create(
            :survey_id => @survey.id,
            :user_id => user_id,
            :options => options
          )
      ...
      survey = Vger::Resources::Exit::Survey.send_survey_to_users(
        :id => @survey.id,
      ...
      if failed_user_surveys.present?
        ...
        redirect_to users_company_exit_survey_path(@company.id, @survey.id)
      end
    end
  end
  ...
  def user
    ...
    @user_surveys = Vger::Resources::Exit::UserSurvey.where(:survey_id => @survey.id, :query_options => {
      :user_id => @user.id
    })
  ...
  def users
    ...
    case order
      when "default"
        order = "exit_user_surveys.completed_at DESC"
      when "id"
        order = "users.id #{order_type}"
      when "name"
        order = "users.name #{order_type}"
      when "status"
        column = "exit_user_surveys.status"
        order = "case when #{column}='scored' then 1 when #{column}='started' then 2 when #{column}='sent' then 3 end, exit_user_surveys.updated_at #{order_type}"
    end
    scope = Vger::Resources::Exit::UserSurvey.where(:survey_id => @survey.id).where(:page => params[:page], :per => 10, :joins => :user, :order => order).where(:include => [:user])
    params[:search] ||= {}
