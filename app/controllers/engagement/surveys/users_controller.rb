class Engagement::Surveys::UsersController < ApplicationController
  ...
  layout 'engagement'
  def bulk_upload
    @s3_key = "engagement/survey_users/#{@survey.id}_#{Time.now.strftime("%d_%m_%Y_%H_%M_%S_%P")}"
    if !params[:bulk_upload] || !params[:bulk_upload][:file]
      flash[:error] = "Please select a xls file."
      redirect_to add_users_bulk_company_engagement_survey_url(company_id: @company.id,id: @survey.id,candidate_stage: params[:candidate_stage]) and return
    end
  ...
  def add_users
    ...
    if request.put?
      ...
      if params[:users].empty?
        flash[:error] = "Please add at least 1 User to send the survey. You may also select 'Add Users Later' to save the survey and return to the Engagement Listings."
      ...
      if !survey_elements.present?
        flash[:error] = "You need to select elements before sending an survey. Please select traits from below."
        redirect_to add_elements_company_engagement_survey_path(:company_id => params[:company_id], :id => params[:id])
      end
    end
  end
  ...
  def send_reminder
    if request.get?
      ...
      @user_survey = Vger::Resources::Engagement::UserSurvey.send_reminder(params.merge(:survey_id => params[:id], :id => params[:user_survey_id]))
      flash[:notice] = "Reminder was sent successfully!"
      redirect_to users_company_engagement_survey_path(:company_id => params[:company_id], :id => params[:id])
    end
  end

  def bulk_send_survey_to_users
    Vger::Resources::Engagement::UserSurvey\
    ...
    if request.put?
      ...
      params[:selected_users].each do |user_id,on|
        ...
        unless user_survey
          ...
          user_survey = Vger::Resources::Engagement::UserSurvey.create(
            :survey_id => @survey.id,
            :user_id => user_id,
            :options => options
          )
      ...
      survey = Vger::Resources::Engagement::Survey.send_survey_to_users(
        :id => @survey.id,
      ...
      if failed_user_surveys.present?
        ...
        redirect_to users_company_engagement_survey_path(@company.id, @survey.id)
      end
    end
  end
  ...
  def user
    ...
    @user_surveys = Vger::Resources::Engagement::UserSurvey.where(:survey_id => @survey.id, :query_options => {
      :user_id => @user.id
    })
  ...
  # sort by status needs a engagement sorting logic where sorting is decided by predefined array of statuses
  def users
    order = params[:order_by] || "default"
    order_type = params[:order_type] || "ASC"
    case order
      when "default"
        order = "engagement_user_surveys.completed_at DESC"
      when "id"
        order = "jombay_users.id #{order_type}"
      when "name"
        order = "jombay_users.name #{order_type}"
      when "status"
        column = "engagement_user_surveys.status"
        order = "case when #{column}='scored' then 1 when #{column}='started' then 2 when #{column}='sent' then 3 end, engagement_user_surveys.updated_at #{order_type}"
    end
    scope = Vger::Resources::Engagement::UserSurvey.where(:survey_id => @survey.id).where(:page => params[:page], :per => 10, :joins => :user, :order => order).where(:include => [:user])
    params[:search] ||= {}
