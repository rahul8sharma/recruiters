class Engagement::Surveys::CandidatesController < ApplicationController
  ...
  layout 'engagement'
  def bulk_upload
    @s3_key = "engagement/survey_candidates/#{@survey.id}_#{Time.now.strftime("%d_%m_%Y_%H_%M_%S_%P")}"
    if !params[:bulk_upload] || !params[:bulk_upload][:file]
      flash[:error] = "Please select a csv file."
      redirect_to add_candidates_bulk_company_engagement_survey_url(company_id: @company.id,id: @survey.id,candidate_stage: params[:candidate_stage]) and return
    end
  ...
  def add_candidates
    ...
    if request.put?
      ...
      if params[:candidates].empty?
        flash[:error] = "Please add at least 1 Candidate to send the survey. You may also select 'Add Candidates Later' to save the survey and return to the Engagement Listings."
    ...
    if request.put?
      ...
      params[:selected_candidates].each do |candidate_id,on|
        ...
        unless candidate_survey
          ...
          candidate_survey = Vger::Resources::Engagement::CandidateSurvey.create(
            :survey_id => @survey.id,
            :candidate_id => candidate_id,
            :options => options
          )
      ...
      survey = Vger::Resources::Engagement::Survey.send_survey_to_candidates(
        :id => @survey.id,
      ...
      if failed_candidate_surveys.present?
        ...
        redirect_to candidates_company_engagement_survey_path(@company.id, @survey.id)
      end
    end
  end
  ...
  def candidates
    ...
    case order
      when "default"
        order = "engagement_candidate_surveys.completed_at DESC"
      when "id"
