class Oac::SubscriptionsController < MasterDataController
  ...
  def expire_subscription
    if params[:subscription_id].blank?  
      ...
      redirect_to manage_oac_subscriptions_path and return
    end
    now = Time.now
    subscription = Vger::Resources::Oac::Subscription.save_existing(params[:subscription_id], :valid_to => now)
    if subscription.error_messages.present?
      flash[:error] = subscription.error_messages.join("<br/>").html_safe
      redirect_to manage_oac_subscriptions_path
    else
      Vger::Resources::Oac::Invitation.update_all(
        :query_options => { 
          :company_id => params[:company_id],
          :subscription_id => params[:subscription_id], 
          :status => Vger::Resources::Oac::Invitation::Status::UNLOCKED 
        }, 
        :update_attributes => { 
          :status => Vger::Resources::Oac::Invitation::Status::EXPIRED, 
          :expiry => now
        }
      )
