class Suitability::PostAssessmentGuidelinesController < ApplicationController
  ...
  def import_from_google_drive
    ...
    redirect_to suitability_post_assessment_guidelines_url, notice: "Import operation queued. Email notification should arrive as soon as the import is complete."
  end

  def export_to_google_drive
    Vger::Resources::Suitability::PostAssessmentGuideline\
      .export_to_google_drive(params[:export].merge(:columns => [:id,:body, :candidate_stage, :factor_id]))
    redirect_to suitability_post_assessment_guidelines_url, notice: "Export operation queued. Email notification should arrive as soon as the export is complete."
  end
