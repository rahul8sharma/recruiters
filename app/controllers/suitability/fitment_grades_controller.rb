class Suitability::FitmentGradesController < ApplicationController
  ...
  def import_from_google_drive
    ...
    redirect_to suitability_fitment_grades_url, notice: "Import operation queued. Email notification should arrive as soon as the import is complete."
  end

  def export_to_google_drive
    Vger::Resources::Suitability::FitmentGrade\
      .export_to_google_drive(params[:export].merge(:columns => [:id,:name, :uid, :min_factors_pass_percent, :max_factors_pass_percent]))
    redirect_to suitability_fitment_grades_url, notice: "Export operation queued. Email notification should arrive as soon as the export is complete."
  end
