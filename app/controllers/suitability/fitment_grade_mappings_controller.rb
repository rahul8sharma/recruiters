class Suitability::FitmentGradeMappingsController < ApplicationController
  ...
  def import_from_google_drive
    ...
    redirect_to suitability_fitment_grade_mappings_url, notice: "Import operation queued. Email notification should arrive as soon as the import is complete."
  end

  def export_to_google_drive
    Vger::Resources::Suitability::FitmentGradeMapping\
      .export_to_google_drive(params[:export].merge(:columns => [:id, :fitment_grades, :overall_fitment_grade_id]))
    redirect_to suitability_fitment_grade_mappings_url, notice: "Export operation queued. Email notification should arrive as soon as the export is complete."
  end
